...
GaussLaguerre::GaussLaguerre(int size)
{
    for (int n = 1; n <= 4; n++) // frequency
    {
        for (int k = 0; k <= 5; k++) // size
        {
            ...
            realKernels.push_back(re);
            imagKernels.push_back(im);
        }
    }
}
...
{
    return pow((double)-1, k) * pow(2, (fabs((double)n)+1.0)/2.0) * pow(M_PI, fabs((double)n)/2.0) *
            pow((((double)(factorial(k)))/factorial(fabs((double)n) + k)), 0.5) * pow((double)r, fabs((double)n)) *
