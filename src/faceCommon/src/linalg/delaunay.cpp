...
std::vector<cv::Vec3i> Delaunay::process(std::vector<cv::Point2d> &points)
{
    int n = points.size();
    ...
    std::map<std::pair<float, float>, int> coord2Index;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        coord2Index[std::pair<float, float>(x, y)] = i;
    }
    ...
    for (unsigned int i = 0; i < triangleList.size(); i++)
    {
        ...
        triangleIndicies[0] = coord2Index[std::pair<float, float>(p1.x, p1.y)];
        triangleIndicies[1] = coord2Index[std::pair<float, float>(p2.x, p2.y)];
        triangleIndicies[2] = coord2Index[std::pair<float, float>(p3.x, p3.y)];

        result.push_back(triangleIndicies);
    }
    return result;
