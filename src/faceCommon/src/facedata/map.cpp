...
void Map::erode(int kernelSize)
{
    if ((kernelSize % 2 != 1) || (kernelSize < 3))
        throw FACELIB_EXCEPTION("invalid kernel size");
    int range = kernelSize/2;

    int count = 0;
    int *toRemoveX = new int[w*h];
    int *toRemoveY = new int[w*h];
...
void Map::add(const Map &other)
{
    if ((w != other.w) || (h != other.h))
        throw FACELIB_EXCEPTION("invalid map size");
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
...
Map Map::densityMap(int kernelSize, bool fromCenter) const
{
    ...
    int r = kernelSize/2;
    double fromCenterToBorder = sqrt((double)(w/2*w/2 + h/2*h/2));
    ...
    for (int y = 0; y < h+r; y++)
    {
        for (int x = 0; x < w+r; x++)
        {
            ...
            if (x >= r && y >= r)
            {
                ...
                if (fromCenter)
                {
                    double toCenter = sqrt((double)((centerX-(w/2))*(centerX-(w/2)) + (centerY-(h/2))*(centerY-(h/2))));
...
std::vector<double> Map::getUsedValues() const
{
...
void Map::applyCvGaussBlur(int size, int times)
{
    cv::Size ksize(size, size);
    for (int i = 0; i < times; i++)
        cv::GaussianBlur(values, values, ksize, 0);
}
